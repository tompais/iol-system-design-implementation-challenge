...
| EC2 host resolution | Dynamic: `aws ec2 describe-instances --instance-ids $EC2_INSTANCE_ID` resolves the current public DNS at deploy time | Static `EC2_HOST` secret (public IP) — rotates on every EC2 stop/start, requiring a manual secret update each time |
...
**EC2 dynamic host resolution:** The CD workflow was storing the EC2 instance's public IP as the `EC2_HOST` secret, which caused `ssh-keyscan` to fail after every EC2 stop/start cycle because AWS reassigns public IPs on restart. Claude replaced the static secret with a two-step resolve: `aws-actions/configure-aws-credentials@v4` configures a minimal IAM identity (single `ec2:DescribeInstances` permission), then `aws ec2 describe-instances --instance-ids $EC2_INSTANCE_ID` retrieves the current public DNS at deploy time. The `EC2_INSTANCE_ID` is immutable — it never changes regardless of stop/start or IP rotation — eliminating the need to manually update secrets after every restart. Step-level output (`steps.ec2.outputs.host`) threads the resolved hostname into both the `ssh-keyscan` and `ssh` steps without storing it in a secret or environment variable that persists beyond the job.
