...
**`capacity`** — the maximum burst size. This is the upper bound on how many requests a single key can make in a very short window (e.g., within a single millisecond). It also caps the tokens that accumulate during idle periods. Setting `capacity: 1` removes burst capacity (the bucket can never hold more than one token at a time), but the sustained requests-per-second limit is still controlled by `refill-rate-per-second`. For example, `capacity: 1` plus `refill-rate-per-second: 1` gives a strict 1-req/sec throttle with no burst, while `capacity: 1` plus `refill-rate-per-second: 5` allows 5 req/sec with no burst. Setting `capacity: 100` allows a burst of 100 requests after an idle period. Mapped to `TokenBucketConfig.capacity` via `@Value("${rate-limiter.capacity}")`.
...
**`milliTokens` integer representation:** Token count is stored as `Long` milliTokens (1 token = 1000 milliTokens) in `BucketState`, rather than `Double` tokens. `AtomicReference.compareAndSet` on `BucketState` operates purely on reference identity — it does not use the `data class` `equals()` or compare field bit patterns — so the choice of `Long` is about arithmetic correctness, not CAS semantics. Integer arithmetic avoids IEEE 754 rounding issues and makes refill and `Retry-After` calculations deterministic and predictable. The 1000× multiplier also preserves sub-token precision: at 5 tokens/sec, 100ms earns 500 milliTokens (half a token), which accumulates correctly until 1000 milliTokens (one full token) is available.
...
| Token precision | `milliTokens` Long (exact integer arithmetic) | Avoids IEEE 754 rounding drift; refill and retry-after results are deterministic | `Double` | Floating-point rounding makes refill calculations non-deterministic; no benefit over integer arithmetic here |
...
In most OOP languages, wrapping a `String` in a class adds a heap allocation and an extra pointer dereference every time the wrapper is used. A Kotlin `value class` is a **compile-time wrapper**: on the JVM it is typically represented as the underlying `String` without extra allocation when used in straightforward contexts (non-nullable, not passed as `Any`, not flowing through erased generics), but it can still be boxed in other situations.

Why use it instead of a plain `String`? Type safety. Without `RateLimitKey`, a function signature like `fun tryConsume(key: String)` gives no indication of what kind of string is expected, and callers could accidentally pass a user ID, a session token, or a URL. With `RateLimitKey`, the type system enforces intent at compile time. The `@JvmInline` annotation opts in to the JVM value-class representation (typically unboxed when not used as nullable/generic/`Any`, but still boxed in those situations). Equality is structural — two `RateLimitKey("user-1")` instances are equal — which is what `ConcurrentHashMap` key lookup requires.
...
| **`@JvmInline value class`** | A Kotlin compile-time wrapper that is usually represented as its underlying type at runtime. It can avoid heap allocation in many call sites, but may still be boxed in generics, nullable, or `Any` contexts (for example as a `ConcurrentHashMap` key). Used for type-safe identifiers (`RateLimitKey`) with minimal runtime overhead. |
...
| **IEEE 754** | The international standard for floating-point arithmetic. Defines how `Double` and `Float` values are represented in binary. Rounding during arithmetic can cause logically equal computations to produce slightly different bit patterns, making calculations non-deterministic — which is why `Long` integer arithmetic is used for refill and retry-after calculations instead. |
